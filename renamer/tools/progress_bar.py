...
async def progress_bar(current, total, status_msg, start, msg):
    ...
    if round((present - start) % 3) == 0 or current == total:
        ...
        progressbar = "[{0}{1}]".format(\
            ''.join([f"{WHITE_SQUARE_CONTAINING_BLACK_SMALL_SQUARE}" for i in range(math.floor(percentage / 10))]),
            ''.join([f"{WHITE_SQUARE_WITH_ROUNDED_CORNERS}" for i in range(10 - math.floor(percentage / 10))])
            )
...
{PAGER} **PROGRESS**: {humanbytes(current)}

{HAMMER_AND_WRENCH} **Size**: {humanbytes(total)}

{STOPWATCH} **ETA**: {time_to_complete}"""
        try:
